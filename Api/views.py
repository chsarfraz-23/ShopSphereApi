...
class AuthUserActionsView(mixins.UpdateModelMixin, viewsets.ReadOnlyModelViewSet):
...
class DeepSeekApiView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        instance.save()
        return Response(
            DeepSeekAPIViewSerializer(instance).data, status=status.HTTP_201_CREATED
        )

    def get(self, request, pk=None):
        if not pk:
            queryset = DeepSeekRequestResponseModel.objects.all().order_by(
                "-created_at"
            )
            paginated_query_set = self.pagination_class.paginate_queryset(
                queryset, request
            )
            serializer = DeepSeekAPIViewSerializer(paginated_query_set, many=True)
            return self.pagination_class.get_paginated_response(serializer.data)

        deepseek_request_response_model = get_object_or_404(
            DeepSeekRequestResponseModel, id=pk
        )
